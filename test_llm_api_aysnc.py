...
def call_llm_api_sync():
    ...
    try:
        # 发送POST请求（设置60秒超时，解决504 gateway核心问题）
...
async def call_llm_api_async():
    """
    异步调用LLM接口（方案2：使用asyncio.to_thread包装同步requests调用）
    参考 services/llm_client.py 的实现方式
    """
    # 在线程池中运行同步方法，保持异步接口兼容性
    return await asyncio.to_thread(call_llm_api_sync)
...
if __name__ == "__main__":
    try:
        # 运行异步函数
        asyncio.run(call_llm_api_async())
    except Exception as e:
        print(f"\n脚本执行失败: {e}")